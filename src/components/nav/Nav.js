    ...
    <ul>
      <li
        onClick={() =>
          scrollToSections(
            refs.filter(
              (ref) => ref.current !== null && ref.current.id === 'services'
            )
          )
        }
      ...
      <li
        onClick={() =>
          scrollToSections(
            refs.filter(
              (ref) => ref.current !== null && ref.current.id === 'work'
            )
          )
        }
      ...
      <li
        onClick={() =>
          scrollToSections(
            refs.filter(
              (ref) => ref.current !== null && ref.current.id === 'contact'
            )
          )
        }
  ...
  const sideMenu = (
    <div
      className={
        navState
          ? `${navStyles.side_menu}`
          : `${navStyles.side_menu} ${navStyles.hidden}`
      }
    >
      {navList}
      <div
        className={navStyles.side_menu_close}
        onClick={() => toggleNav(!navState)}
      ></div>
    </div>
  );
    ...
    <nav className={navStyles.nav}>
      ...
      {isMobile ? (
        ...
        <div className={navStyles.nav__right}>
          {location === '/store' ? storeNav : navList}{' '}
        </div>
      )}
    </nav>
  );
